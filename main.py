...
FILE_PATH = r"D:\RAG_MODEL\Danyeus.pdf"
...
def split_pdf_into_batches(input_path, pages_per_batch=10):
...
if __name__ == "__main__":
    ...
    batch_files = split_pdf_into_batches(FILE_PATH, pages_per_batch=10)
    ...
    # KHI CẦN TRUY VẤN (TÌM KIẾM THỬ):
    print("\n" + "="*20 + " TÌM KIẾM THỬ NGHIỆM " + "="*20)
    query_text = "Mục tiêu tháng 4?"
    
    # Nhúng câu hỏi thành vector
    query_response = ollama.embed(model='qwen3-embedding:0.6b', input=query_text)
    query_vector = query_response['embeddings'][0]
    
    # Truy vấn ChromaDB để lấy 2 chunk gần giống nhất
